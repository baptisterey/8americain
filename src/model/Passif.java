...
public class Passif implements Strategie {
	...
	public Integer[] choisirDataDonner(Joueur joueurCourant) {
		...
		for (int i = 1; i < joueurCourant.getMain().size(); i++) {
			if (joueurCourant.getMain().get(indexCarteChoisie).getEffet() instanceof EffetAttaque || joueurCourant
					.getMain().get(indexCarteChoisie).getEffet() instanceof EffetContrerChangerCouleur) {
				indexCarteChoisie = i;
			}
		}

		int indexJoueurChoisi = 0;
		for (int j = 1; j < Jeu.getInstance().getJoueurs().size() - 1; j++) { // le joueur courant est en derni�re
